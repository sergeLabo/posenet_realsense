...
def main():
    ...
    def render_overlay(engine, output, src_size, inference_box):
        ...
        # RIGHT_WRIST
        velocities = {}
        leftp, rightp = None, None
        for pose in poses:
            # #Pose( <KeypointType.LEFT_WRIST: 9>: Keypoint(point=Point(x=414.6617, y=479.0618), score=0.0038025412),
            # #print(pose.pose.keypoints.keys)  #.LEFT_WRIST)
            for label, keypoint in pose.keypoints.items():
                if label == KeypointType.LEFT_WRIST:
                    leftp = keypoint
                if label == KeypointType.RIGHT_WRIST:
                    rightp = keypoint

            if not (leftp and rightp):
                continue
            print(leftp, rightp)
            identity = IDENTITIES[pose.id % len(IDENTITIES)]
            left = 1 - leftp.point[0] / 480  # engine.image_height
            right = 1 - rightp.point[0] / 480  # engine.image_height
            print(left, right)
